<template>
  <div class="index" v-loading="loading">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(item,index) in photoList" :key="index" @click="get_face_by_id(item.id)">
            <td>{{ item.id }}</td>
            <td>
              <span v-if="item.event_type">陌生人</span>
              <span v-else>员工/已登记访客</span>
            </td>
            ...
            <td>
              <img
                v-preview=" item.face_img_ids "
                class="td-img"
                v-lazy=" item.face_img_ids "
                alt
            ...
            <td>{{ item.remark }}</td>
            <td class="font_bold">
              <span v-if=" item.checked ">处理完成</span>
              <span v-else style="color: #ff0000;" class="underline">
                未处理
                <i class="radius bg_red"></i>
              </span>
            </td>
          </tr>
        </tbody>
      ...
      <div class="item-box">
        ...
        <div class="inline_block wid_49 item">
          ...
          <p class="inline_block">
            <span class="color_blue" v-if=" singleData.checked ">已处理完成</span>
            <span v-else class="handle color_red underline" @click="handleVisible = true">
              未处理
              <i class="radius bg_red"></i>
            </span>
          </p>
        ...
        <div class="inline_block item">陌生人图片：</div>
    ...
    <div v-show="tipsVisible" class="tipsModal">
      ...
      <div class="tipsFooter">位置：{{ tipsData.camera_name }}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    get_face_by_id(id) {
      let that = this,
        url = this.$api.api_duty + "camera/face_event/get/" + id;
      ...
      that.$axios
        ...
        .then(function(res) {
          if (res.data.message == "success") {
            res.data.data.camera_name =
              res.data.data.camera_name.split("_")[0] +
              "酒店" +
              res.data.data.camera_name.split("_")[1] +
              "栋" +
              res.data.data.camera_name.split("_")[2] +
              "层" +
              res.data.data.camera_name.split("_")[3];
            ...
            if (res.data.data.photo_ids) {
              ...
              for (let i in res.data.data.photo_ids.split(",")) {
                url.push(
                  that.$api.api_img +
                    "camera_face_event/" +
                    res.data.data.photo_ids.split(",")[i] +
                    ".jpg"
                );
              }
            ...
            if (res.data.data.face_img_ids) {
              ...
              for (let i in res.data.data.face_img_ids.split(",")) {
                face_url.push(
                  that.$api.api_img +
                    "camera_face_event/" +
                    res.data.data.face_img_ids.split(",")[i] +
                    ".jpg"
                );
              }
    ...
    update_by_id(id) {
      let that = this,
        url = this.$api.api_duty + "camera/face_event/update/" + id;
      that.$axios
        .post(url, { remark: that.remark, checked: 1 })
        .then(function(res) {
          if (res.data.message == "success") {
    ...
    handleCurrentPageChange(currentPage, event_type) {
      let that = this,
        url = this.$api.api_duty + "camera/face_event/list";
      /* websocket */
      that.initWebSocket();
        ...
        .then(function(res) {
          if (res.data.message == "success") {
            for (let i in res.data.data.list) {
              res.data.data.list[i].camera_name =
                res.data.data.list[i].camera_name.split("_")[0] +
                "酒店" +
                res.data.data.list[i].camera_name.split("_")[1] +
                "栋" +
                res.data.data.list[i].camera_name.split("_")[2] +
                "层" +
                res.data.data.list[i].camera_name.split("_")[3];
              if (
                res.data.data.list[i].photo_ids &&
                res.data.data.list[i].photo_ids.split(",").length
              ) {
                res.data.data.list[i].url_img =
                  that.$api.api_img +
                  "camera_face_event/" +
                  res.data.data.list[i].photo_ids.split(",")[0] +
                  ".jpg";
                res.data.data.list[i].face_img_ids =
                  that.$api.api_img +
                  "camera_face_event/" +
                  res.data.data.list[i].face_img_ids +
                  ".jpg";
            ...
            res.data.data.message === "authorization invalid" &&
            res.data.data.message === "fail"
          ) {
            that.websocketclose();
    ...
    initWebSocket() {
      //初始化weosocket
      const wsurl =
        this.$api.api_ws + "/ws/miss/" + this.hotelInfo.hotel_id + "/"; //ws地址
      // console.log(wsurl);
    ...
    websocketonmessage(e) {
      ...
      if (data) {
        ...
        data.camera_name =
          data.camera_name.split("_")[0] +
          "酒店" +
          data.camera_name.split("_0")[1] +
          "栋" +
          data.camera_name.split("_")[2] +
          "层" +
          data.camera_name.split("_")[3] +
          "区";
        if (data.photo_ids && data.photo_ids.split(",").length) {
          data.url_img =
            that.$api.api_img +
            "camera_face_event/" +
            data.photo_ids.split(",")[0] +
            ".jpg";
        } else {
          data.url_img =
            that.$api.api_img + "camera_face_event/" + data.photo_ids + ".jpg";
        }
